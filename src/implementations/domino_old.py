...
#         G_modularity=read_graphml(cache_file)
        G_modularity=pickle.load(open(cache_file, 'rb')) 
        # print(G_modularity.nodes(data=True))
        for n in G_modularity:
            # print(G_modularity.nodes[n].items())
    ...
    G_modularity_copy=G_modularity.copy()
#     for n in G_modularity_copy.nodes:
#         G_modularity_copy.nodes[n]['pertubed_node']=str(G_modularity_copy.nodes[n]['pertubed_node']) 
#     write_graphml(G_modularity_copy, cache_file)
    pickle.dump(G_modularity, open(cache_file, 'wb+'))
    return G_modularity

def get_pcst_prize(G_cc, prize_factor, n_steps):
    prizes={}
    p_cc = linear_threshold(G_cc, [n for n in G_cc.nodes if G_cc.nodes[n]['pertubed_node']], steps=n_steps)
...
def get_permuted_scores(G_cc, labels, n_steps, nodes, prize_factor, n_permutations):
    ...
    for i_iteration in np.arange(n_permutations):
        ...
        lt_genes = linear_threshold(p_cc, [n for n in p_cc.nodes if p_cc.nodes[n]['pertubed_node']], steps=n_steps)
        for p_node in G_cc.nodes:
...
#     G = add_scores_to_nodes(G, scores)
    ...
    cache_file=os.path.split(network_file)[1].split(".")[0]+"."+os.path.split(slices_file)[1].split(".")[0]+".pkl"  # +"_"+active_genes_file.split('/')[-2].split("_")[-1]+
    print(f'cache_file_name: {cache_file}')
    G_modularity=prune_network_by_modularity(G, modularity_connected_components, os.path.join(os.path.split(slices_file)[0],cache_file))
