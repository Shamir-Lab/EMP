...
def get_putative_modules(G, full_G=None, improvement_delta=0, modularity_score_objective=1, module_threshold=0.05, n_cc=1.0):
    ...
    is_enriched_sublice = (len(G_optimized.nodes)<100) or len(G_optimized.nodes)==0 # sig_score<module_threshold and 
...
def retain_relevant_slices(G_original, module_sig_th):
    ...
    pf_percentile=max(max(pertubed_nodes_in_ccs)*0.8,2) # max(np.percentile(pertubed_nodes_in_ccs, 97, interpolation='higher')*0.9,2) # max(np.percentile(pertubed_nodes_in_ccs, 90, interpolation='higher'),2)
    print(f'pf_percentile: {pf_percentile}')
    perturbation_factor=pf_percentile
...
#     if len(cur_cc) < 4 or n_pertubed_nodes==0 or not (len(pertubed_nodes_in_cc) > 4 or len(pertubed_nodes_in_cc)/float(len(cur_cc))>=0.2 or len(pertubed_nodes_in_cc)/float(n_pertubed_nodes)>=0.05):
    # if len(cur_cc) < 4 or n_pertubed_nodes==0 or not (len(pertubed_nodes_in_cc) >= perturbation_factor): 
    if len(cur_cc) < 4 or not (len(pertubed_nodes_in_cc) >= perturbation_factor):
        # print("false")
        return None
    else:
        # print("true")
...
def analyze_slice(params):
    ...
    n_pertubed_nodes=sum([G_cc.nodes[a]["pertubed_node"] for a in G_cc.nodes])
    print(f'active gene ratio: {n_pertubed_nodes}/{len(G_cc.nodes)}')
    prize_factor=0#0.7*(1-n_pertubed_nodes/float(len(G_cc.nodes)))
    ...
    print(f'ratio of retained slice: {len(G_subslice.edges)}/{len(G_cc.edges)}   {len(G_subslice.nodes)}/{len(G_cc.nodes)}')
...
def get_final_modules(G, G_putative_modules):
    ...
    fdr_bh_results = fdrcorrection0(sig_scores, alpha=0.05, method='indep', is_sorted=False) 
    for i_cur_module, cur_G_module in enumerate(G_putative_modules):
        if sig_scores[i_cur_module]/len(sig_scores)<0.05: # fdr_bh_results[0][i_cur_module]:
            module_sigs.append((cur_G_module, sig_scores[i_cur_module]/len(sig_scores)))# fdr_bh_results[1][i_cur_module]))
        
    

    print(f'{min([-1]+list(sig_scores))} {fdr_bh_results}')

    module_sigs = sorted(module_sigs, key=lambda a: a[1])
    return [a[0] for a in module_sigs]

def main(active_genes_file, network_file, slices_file=None, slice_threshold=0.3, module_threshold=0.05, prize_factor=0.7, n_steps=20, n_permutations=10000):
    ...
    print(f'n of final modules: {len(final_modules)}') # :\n{[list(g.nodes) for g in final_modules]}')
